...
### iceberg 数据生成。 
 ...
 2. 创建iceberg格式的表
    ```
    create table local.db.wen1(id bigint, data string) using iceberg
    ```
 3. 插入数据
    ```
...
Refer Doc: https://iceberg.apache.org/spec/

上面的元数据文件之间是什么关系，我们结合Iceberg的文件系统架构进行简单介绍。 

![](/images/bigdata/iceberg-filesystem.webp)
...
| added_data_files_count    | 添加manifest file文件数            |
| existing_data_files_count | 继承的manifest file 文件数         |
| deleted_data_files_count  | 删除的manifest file 文件数         |
| added_rows_count          | 数据增加的rows数                   |
| delete_rows_count         | 数据删除的rows数                   |
...
| 字段                          | 作用                                      |
| ----------------------------- | ----------------------------------------- |
|status| 标记文件状态 0: EXISTING 1: ADDED 2: DELETED |
